...
//
float Noise3D( const float3 referencePos, const float referenceLOD, float coneWidth, float coneWidthProjected, float noiseBias, out float noise )
{
    ...
    noise = frac( noise + noiseBias ); //Hash2D( float2( noise, g_globals.Noise.x ) );
...
// https://casual-effects.com/research/Wyman2017Hashed/Wyman2017Hashed.pdf
//
void Noise3D( float3 worldCoord, float noiseBias, out float noise, out float noiseAttenuation )
{
...
float3 R3seq(float s)
{
    const float g = 1.22074408460575947536;
    float a1 = 1.0f / g, a2 = 1.0f / (g * g), a3 = 1.0f / (g * g * g);
    return float3(float(fmod((0.5f + a1 * s), 1.0f)), float(fmod((0.5f + a2 * s), 1.0)), float(fmod((0.5f + a3 * s), 1.0)));
}
