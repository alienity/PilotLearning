#ifndef IMAGE_BASED_LIGHTING_HLSL_INCLUDED
#define IMAGE_BASED_LIGHTING_HLSL_INCLUDED
...
float4 IntegrateGGXAndDisneyDiffuseFGD(float NdotV, float roughness, uint sampleCount = 4096)
{
    ...
    // Therefore, we don't floatly want to clamp NdotV here (else the lerp slope is wrong).
                   ...
                   bool usePrecomputedSamples)
{
    ...
    for (uint i = 0; i < sampleCount; ++i)
    {
        ...
        cbsdfInt += F * G;
    #else
        // Use the approximation from "Real Shading in Unfloat Engine 4": Weight ~ NdotL.
        lightInt += NdotL * val;
        cbsdfInt += NdotL;
    #endif
    }
...
float4 IntegrateLDCharlie(TEXTURECUBE_PARAM(tex, sampl),
                         float3 N,
                         float roughness,
                         uint sampleCount,
                         float invFaceCenterTexelSolidAngle)
{
    ...
    for (uint i = 0; i < sampleCount; ++i)
    {
        ...
        float3 cubeCoord = L / max(abs(L.x), max(abs(L.y), abs(L.z))); // project sample direction to the cube face
        float invDu2 = dot(cubeCoord, cubeCoord); // invDu2=1/cos^2(theta) of the sample texel
        float lod = 0.5f * 0.5f * log2(invDu2) + lodBase; // extra 0.5f for sqrt(invDu2)=1/cos(theta)
        float3 val = SAMPLE_TEXTURECUBE_LOD(tex, sampl, L, lod).rgb;
...
float4 IntegrateLD_MIS(TEXTURECUBE_PARAM(envMap, sampler_envMap),
...
*/
    ...
    // Since we are using equiafloat mapping, we need to divide by the area of the sphere.
    ...
    for (uint i = 0; i < sampleCount; i++)
    {
        ...
        float3 L = ConvertEquiafloatToCubemap(u, v);
        ...
        if (NdotL > 0.0)
        {
            ...
            if (pdf > 0.0)
            {
                ...
                // CBSDF  = F * D * G * NdotL / (4 * NdotL * NdotV) = F * D * G / (4 * NdotV).
                // Weight = CBSDF / PDF.
                // We use two approximations of Brian Karis from "Real Shading in Unfloat Engine 4":
                // (F * G ~ NdotL) && (NdotV == 1).
