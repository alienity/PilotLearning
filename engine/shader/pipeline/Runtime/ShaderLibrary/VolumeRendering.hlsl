...
float4 LinearizeRGBA(float4 value)
{
    ...
    float a = value.a;
    float d = -log(1 - a);
    float r = (a >= FLT_EPS) ? (d * rcp(a)) : 1; // Prevent numerical explosion
                           ...
                           float cosZenith, float startHeight)
{
    ...
    float rcpH       = heightExponents.x;
    float Z          = cosZenith;
    float absZ       = max(abs(cosZenith), FLT_EPS);
    float rcpAbsZ    = rcp(absZ);

    float minHeight  = (Z >= 0) ? startHeight : -rcp(FLT_EPS);
    float h          = max(minHeight - baseHeight, 0);
...
bool SampleHenyeyGreenstein(float3 incomingDir,
    float anisotropy,
    float3 inputSample,
    ...
    out float pdf)
{
    ...
    {
        float sqrTerm = (1.0 - g * g) / (1.0 - g + 2.0 * g * inputSample.x);
        cosTheta = (1.0 + g * g - sqrTerm * sqrTerm) / (2 * g);
    }

    // Compute direction
    float sinTheta = sqrt(max(0.0, 1.0 - cosTheta * cosTheta));
    float phi = 2.0 * PI * inputSample.y;

    float3 wc = normalize(incomingDir);
    float3x3 coordsys = GetLocalFrame(wc);
                                   ...
                                   out float t, out float sqDist, out float rcpPdf)
{
    ...
    float sqD  = max(rayToLightSqDist + lightSqRadius, FLT_EPS);
