...
float2 ClampAndScaleForBilinearWithCustomScale(float2 uv, float2 scale, float4 screenSize)
{
    float2 maxCoord = 1.0f - screenSize.zw;
    return min(uv, maxCoord) * scale;
}

float3 Fetch(TEXTURE2D_PARAM(tex, LinearClampSampler), float2 coords, float2 offset, float2 scale, float4 screenSize)
{
    float2 uv = (coords + offset * screenSize.zw);
    uv = ClampAndScaleForBilinearWithCustomScale(uv, scale, screenSize);
    return SAMPLE_TEXTURE2D_LOD(tex, LinearClampSampler, uv, 0).xyz;
}

float4 Fetch4(TEXTURE2D_PARAM(tex, LinearClampSampler), float2 coords, float2 offset, float2 scale, float4 screenSize)
{
    float2 uv = (coords + offset * screenSize.zw);
    uv = ClampAndScaleForBilinearWithCustomScale(uv, scale, screenSize);
    return SAMPLE_TEXTURE2D_LOD(tex, LinearClampSampler, uv, 0);
}
...
float2 GetClosestFragmentOffset(Texture2D<float> DepthTexture, int2 positionSS)
{
    float center = LOAD_TEXTURE2D_LOD(DepthTexture, positionSS, 0).r;
    ...
    float s3 = LOAD_TEXTURE2D_LOD(DepthTexture, positionSS + offset3, 0).r;
    float s2 = LOAD_TEXTURE2D_LOD(DepthTexture, positionSS + offset2, 0).r;
    float s1 = LOAD_TEXTURE2D_LOD(DepthTexture, positionSS + offset1, 0).r;
    float s0 = LOAD_TEXTURE2D_LOD(DepthTexture, positionSS + offset0, 0).r;
...
float2 GetClosestFragmentCompute(Texture2D<float> DepthTexture, float2 positionSS)
{
    float center = LoadCameraDepth(DepthTexture, positionSS);
    float nw = LoadCameraDepth(DepthTexture, positionSS + int2(-1, -1));
    float ne = LoadCameraDepth(DepthTexture, positionSS + int2(1, -1));
    float sw = LoadCameraDepth(DepthTexture, positionSS + int2(-1, 1));
    float se = LoadCameraDepth(DepthTexture, positionSS + int2(1, 1));
...
float2 GetMotionVector(Texture2D<float4> CameraMotionVectorsTexture, Texture2D<float> DepthTexture, SAMPLER(PointClampSampler), float2 uv, int2 positionSS, float4 inputSize)
{
    float2 motionVector;
    ...
    DecodeMotionVector(SAMPLE_TEXTURE2D_LOD(CameraMotionVectorsTexture, PointClampSampler,
        ClampAndScaleUVForPoint(float4(1.0f, 1.0f, 1.0f, 1.0f), uv + closestOffset * inputSize.zw), 0), motionVector);
#endif
    return motionVector;
}

float ModifyBlendWithMotionVectorRejection(TEXTURE2D(VelocityMagnitudeTexture), float mvLen, float2 prevUV, float blendFactor, float speedRejectionFactor, float2 rtHandleScale)
{
...
CTYPE HistoryBilinear(TEXTURE2D(HistoryTexture), SAMPLER(PointClampSampler), float2 UV, float2 rtHandleScale, float4 screenSize)
{
    CTYPE color = Fetch4(HistoryTexture, PointClampSampler, UV, 0.0, rtHandleScale, screenSize).CTYPE_SWIZZLE;
    return color;
}
...
CTYPE HistoryBicubic5Tap(TEXTURE2D(HistoryTexture), SAMPLER(PointClampSampler),
    float2 UV, float sharpening, float4 historyBufferInfo, float2 rtHandleScale, float4 screenSize)
{
    ...
    CTYPE s0 = Fetch4(HistoryTexture, PointClampSampler, float2(tc12.x, tc0.y), 0.0, rtHandleScale, screenSize).CTYPE_SWIZZLE;
    CTYPE s1 = Fetch4(HistoryTexture, PointClampSampler, float2(tc0.x, tc12.y), 0.0, rtHandleScale, screenSize).CTYPE_SWIZZLE;
    CTYPE s2 = Fetch4(HistoryTexture, PointClampSampler, float2(tc12.x, tc12.y), 0.0, rtHandleScale, screenSize).CTYPE_SWIZZLE;
    CTYPE s3 = Fetch4(HistoryTexture, PointClampSampler, float2(tc3.x, tc0.y), 0.0, rtHandleScale, screenSize).CTYPE_SWIZZLE;
    CTYPE s4 = Fetch4(HistoryTexture, PointClampSampler, float2(tc12.x, tc3.y), 0.0, rtHandleScale, screenSize).CTYPE_SWIZZLE;
...
CTYPE GetFilteredHistory(TEXTURE2D(HistoryTexture), SAMPLER(PointClampSampler),
    float2 UV, float sharpening, float4 historyBufferInfo, float2 rtHandleScale, float4 screenSize)
{
    CTYPE history = 0;
...
void GatherNeighbourhood(TEXTURE2D(InputTexture), SAMPLER(PointClampSampler),
    float2 UV, float2 positionSS, CTYPE centralColor, float2 rtHandleScale, float4 screenSize, out NeighbourhoodSamples samples)
{
    ...
    for (int i = 0; i < NEIGHBOUR_COUNT; ++i)
    {
        ...
        offsetIndex += 4;
#endif
        samples.neighbours[i] = ConvertToWorkingSpace(
            Fetch4(InputTexture, PointClampSampler, UV, NeighbourOffsets[offsetIndex], rtHandleScale, screenSize).CTYPE_SWIZZLE);
    }
