...
float ScramblingValueFloat(Texture2D<float2> _ScramblingTexture, uint2 pixelCoord)
{
    pixelCoord = pixelCoord & 255;
    return _ScramblingTexture[uint2(pixelCoord.x, pixelCoord.y)].x;
}

float2 ScramblingValueFloat2(Texture2D<float2> _ScramblingTexture, uint2 pixelCoord)
{
    pixelCoord = pixelCoord & 255;
    return _ScramblingTexture[uint2(pixelCoord.x, pixelCoord.y)].xy;
}

uint ScramblingValueUInt(Texture2D<float2> _ScramblingTexture, uint2 pixelCoord)
{
    pixelCoord = pixelCoord & 255;
    return clamp((uint)(_ScramblingTexture[uint2(pixelCoord.x, pixelCoord.y)].x * 256.0), 0, 255);
}

uint2 ScramblingValueUInt2(Texture2D<float2> _ScramblingTexture, uint2 pixelCoord)
{
    pixelCoord = pixelCoord & 255;
...
float GetLDSequenceSampleFloat(Texture2D<float> _OwenScrambledTexture, uint sampleIndex, uint sampleDimension)
{
...
uint GetLDSequenceSampleUInt(Texture2D<float> _OwenScrambledTexture, uint sampleIndex, uint sampleDimension)
{
    // Make sure arguments are in the right range
    sampleIndex = sampleIndex & 255;
    // sampleDimension = sampleDimension & 255;
...
float GetBNDSequenceSample(
    ...
    uint2 pixelCoord, uint sampleIndex, uint sampleDimension)
{
    // wrap arguments
