...
{
    uint m_SourceTexIndex;
    uint m_ExposureIndex;
    uint m_BloomResultIndex;
    uint m_LumaResultIndex;
}
...
{
    Texture2D<float3>       SourceTex   = ResourceDescriptorHeap[m_SourceTexIndex];
    StructuredBuffer<float> Exposure    = ResourceDescriptorHeap[m_ExposureIndex];
    RWTexture2D<float3>     BloomResult = ResourceDescriptorHeap[m_BloomResultIndex];
    RWTexture2D<uint>       LumaResult  = ResourceDescriptorHeap[m_LumaResultIndex];
