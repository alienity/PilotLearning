...
float3x3 WorldToProxySpace(EnvLightData lightData)
{
    return transpose(
        float3x3(
            lightData.proxyRight.xyz,
            lightData.proxyUp.xyz,
            lightData.proxyForward.xyz
        )
...
{
    float3 influenceExtents = lightData.influenceExtents.xyz;
    ...
    float3 negativeFalloff = negativeDistance / max(0.0001, lightData.blendDistanceNegative.xyz);
    float3 positiveFalloff = positiveDistance / max(0.0001, lightData.blendDistancePositive.xyz);
    ...
    float3 belowPositiveInfluenceNormalVolume = positiveDistance / max(0.0001, lightData.blendNormalDistancePositive.xyz);
    float3 aboveNegativeInfluenceNormalVolume = negativeDistance / max(0.0001, lightData.blendNormalDistanceNegative.xyz);
    ...
    // We consider R.x as cos(X) and then fade as angle from 60 deg (=acos(1/2)) to 75 deg (=acos(1/4))
...
float3x3 WorldToInfluenceSpace(EnvLightData lightData)
{
    return transpose(
        float3x3(
            lightData.influenceRight.xyz,
            lightData.influenceUp.xyz,
            lightData.influenceForward.xyz
        )
    ); // worldToLocal assume no scaling
}
...
{
    float3 positionIS = positionWS - lightData.influencePositionRWS.xyz;
