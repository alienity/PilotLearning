...
static float AngleBetween(const glm::float3& dir0, const glm::float3& dir1)
{
    return std::acos(std::max(glm::dot(dir0, dir1), 0.00001f));
}

void SkyCache::Init(glm::float3 sunDirection, glm::float3 groundAlbedo, float turbidity)
{
    sunDirection.y = MoYu::Saturate(sunDirection.y);
    sunDirection = glm::normalize(sunDirection);
    turbidity = MoYu::Clamp(turbidity, 1.0f, 32.0f);
    groundAlbedo = MoYu::Saturate(groundAlbedo);
    ...
    float thetaS = AngleBetween(sunDirection, glm::float3(0, 1, 0));
    float elevation = MoYu::f::PI - thetaS;
    ...
    if(skyCache.CubeMap == nullptr)
    {
        ...
        for(uint64 s = 0; s < 6; ++s)
        {
            for(uint64 y = 0; y < CubeMapRes; ++y)
            {
                ...
                {
                    glm::float3 dir = MapXYSToDirection(x, y, s, CubeMapRes, CubeMapRes);
                    glm::float3 radiance = SampleSky(skyCache, dir);
...
glm::float3 Skybox::SampleSky(const SkyCache& cache, glm::float3 sampleDir)
{
    Assert_(cache.StateR != nullptr);

    float gamma = AngleBetween(sampleDir, cache.SunDirection);
    float theta = AngleBetween(sampleDir, glm::float3(0, 1, 0));
