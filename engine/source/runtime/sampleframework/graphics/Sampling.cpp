...
#define RadicalInverse_(base) \
    { \
        ...
        float factor = 1.0f; \
        while(sampleIdx) { \
            glm::uint64 next  = sampleIdx / base; \
            glm::uint64 digit = sampleIdx - next * base; \
...
float RadicalInverseFast(glm::uint64 baseIdx, glm::uint64 sampleIdx)
{
    assert(baseIdx < 64);
...
static glm::uint32 CMJPermute(glm::uint32 i, glm::uint32 l, glm::uint32 p)
{
    glm::uint32 w = l - 1;
    w |= w >> 1;
    w |= w >> 2;
...
static float CMJRandFloat(glm::uint32 i, glm::uint32 p)
{
    i ^= p;
    i ^= i >> 17;
    i ^= i >> 10; i *= 0xb36534e5;
...
{
    glm::int32 N = numSamplesX * numSamplesY;
    sampleIdx = CMJPermute(sampleIdx, N, pattern * 0x51633e2d);
    glm::int32 sx = CMJPermute(sampleIdx % numSamplesX, numSamplesX, pattern * 0x68bc21eb);
    glm::int32 sy = CMJPermute(sampleIdx / numSamplesX, numSamplesY, pattern * 0x02e5be93);
    float jx = CMJRandFloat(sampleIdx, pattern * 0x967a889b);
    float jy = CMJRandFloat(sampleIdx, pattern * 0x368cc8b7);
    return glm::float2((sx + (sy + jx) / numSamplesY) / numSamplesX, (sampleIdx + jy) / N);
}

void GenerateCMJSamples2D(glm::float2* samples, glm::uint64 numSamplesX, glm::uint64 numSamplesY, glm::uint32 pattern)
{
    const glm::uint64 numSamples = numSamplesX * numSamplesY;
    for(glm::uint64 i = 0; i < numSamples; ++i)
        samples[i] = SampleCMJ2D(glm::int32(i), glm::int32(numSamplesX), glm::int32(numSamplesY), glm::int32(pattern));
}
