...
std::int64_t Stopwatch::Frequency = [] {
...
Stopwatch::Stopwatch() noexcept : StartTime(GetTimestamp()), Period(1.0 / static_cast<double>(Frequency)) {}

double Stopwatch::GetDeltaTime() const noexcept { return DeltaTime; }
...
{
    std::int64_t Now = GetTimestamp();
    return static_cast<double>(Now - StartTime) * Period;
}

bool Stopwatch::IsPaused() const noexcept { return Paused; }
