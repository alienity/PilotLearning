...
template<std::uint32_t MaxStackSizeInBytes>
class InlineAllocator
{
public:
    InlineAllocator() noexcept : Buffer {}, SizeInBytes(0) {}
	...
	[[nodiscard]] std::uint32_t  GetSize() const noexcept { return SizeInBytes; }
	...
	std::byte* Allocate(std::uint32_t SizeInBytes)
    {
...
public:
    ...
    Delegate(Delegate&& Delegate) noexcept :
        Allocator(std::exchange(Delegate.Allocator, {})), Object(Allocator.GetAllocation()),
...
private:
    Delegate(void* Object, ProxyType Proxy) noexcept : Object(Object), Proxy(Proxy) {}
...
public:
    ...
    MulticastDelegate(bool ThreadSafe = false) : OptMutex(ThreadSafe ? std::make_unique<std::mutex>() : nullptr) {}
