...
#define CommonRootSigDesc(rootSigName) \
    ...
    void VolumeCloudPass::preCompute(RHI::RenderGraph& graph, ImportGraphHandle& graphHandle)
    {
    ...
    void VolumeCloudPass::updateAtmosphere(RHI::RenderGraph& graph, ImportGraphHandle& graphHandle, DrawInputParameters& passInput, DrawOutputParameters& passOutput)
    {
        if (!hasPrecomputed)
            return;
