...
struct RootSignatures
{
    ...
    static void Compile(RHI::D3D12Device* Device, RHI::RenderGraphRegistry& Registry)
    {
        ...
        GBuffer = Registry.CreateRootSignature(Device->CreateRootSignature(RHI::RootSignatureDesc()
        ...
        Meshlet = Registry.CreateRootSignature(Device->CreateRootSignature(RHI::RootSignatureDesc()
                                                                               .Add32BitConstants<0, 0>(1)
                                                                               .AddShaderResourceView<0, 0>()
                                                                               .AddShaderResourceView<1, 0>()
                                                                               .AddShaderResourceView<2, 0>()
        ...
        BloomMask = Registry.CreateRootSignature(
            Device->CreateRootSignature(RHI::RootSignatureDesc().Add32BitConstants<0, 0>(5)));

        BloomDownsample = Registry.CreateRootSignature(
            Device->CreateRootSignature(RHI::RootSignatureDesc().Add32BitConstants<0, 0>(7)));
        ...
        BloomUpsampleBlur = Registry.CreateRootSignature(
            Device->CreateRootSignature(RHI::RootSignatureDesc().Add32BitConstants<0, 0>(6)));

        Tonemap = Registry.CreateRootSignature(Device->CreateRootSignature(RHI::RootSignatureDesc()
                                                                               .Add32BitConstants<0, 0>(6)
                                                                               .AllowResourceDescriptorHeapIndexing()
                                                                               .AllowSampleDescriptorHeapIndexing()));

        BayerDither = Registry.CreateRootSignature(
            Device->CreateRootSignature(RHI::RootSignatureDesc().Add32BitConstants<0, 0>(2)));
    ...
    static void Compile(RHI::D3D12Device* Device, RHI::RenderGraphRegistry& Registry)
    {
        ...
        }
        {
            ...
            IndirectCullMeshShader =
                Registry.CreatePipelineState(Device->CreatePipelineState(L"IndirectCullMeshShader", Stream));
        }
        {
            RHIDepthStencilState DepthStencilState;
            DepthStencilState.DepthEnable = true;
        ...
        }
        {
            ...
            BloomUpsampleBlur =
                Registry.CreatePipelineState(Device->CreatePipelineState(L"Bloom Upsample Blur", Stream));
        }
        {
            struct PsoStream
            {
...
struct RaytracingPipelineStates
{
    ...
    static void Compile(RHI::D3D12Device* Device, RHI::RenderGraphRegistry& Registry)
    {
        ...
        LocalHitGroupRS = Registry.CreateRootSignature(
            Device->CreateRootSignature(RHI::RootSignatureDesc()
        ...
        RTPSO              = Registry.CreateRaytracingPipelineState(Device->CreateRaytracingPipelineState(
            RHI::RaytracingPipelineStateDesc()
                ...
                .AddRootSignatureAssociation(Registry.GetRootSignature(LocalHitGroupRS)->GetApiHandle(),
                                             {g_HitGroupExport})
                .SetGlobalRootSignature(Registry.GetRootSignature(GlobalRS)->GetApiHandle())
                .SetRaytracingShaderConfig(PayloadSize, MOYU_RHI_D3D12_BUILTIN_TRIANGLE_INTERSECTION_ATTRIBUTES)
