...
namespace RHI
{
    D3D12CommandQueue::D3D12CommandQueue(D3D12LinkedDevice* Parent, RHID3D12CommandQueueType Type) :
        D3D12LinkedDeviceChild(Parent), CommandListType(RHITranslateD3D12(Type)), CommandQueue([&] {
            Microsoft::WRL::ComPtr<ID3D12CommandQueue> CommandQueue;
            D3D12_COMMAND_QUEUE_DESC                   Desc = {CommandListType,
    ...
    UINT64 D3D12CommandQueue::Signal() { return Fence.Signal(this); }

	bool D3D12CommandQueue::IsFenceComplete(UINT64 FenceValue) { return Fence.IsFenceComplete(FenceValue); }

    void D3D12CommandQueue::HostWaitForValue(UINT64 FenceValue) { Fence.HostWaitForValue(FenceValue); }
    
    void D3D12CommandQueue::Wait(D3D12CommandQueue* CommandQueue) const { WaitForSyncHandle(CommandQueue->SyncHandle); }
    ...
    D3D12SyncHandle
    D3D12CommandQueue::ExecuteCommandLists(std::vector<D3D12CommandListHandle* const> CommandListHandles,
                                           bool                                        WaitForCompletion)
    {
        ...
        if (NumBarrierCommandList > 0)
        {
            ResourceBarrierCommandAllocatorPool.DiscardCommandAllocator(
                std::exchange(ResourceBarrierCommandAllocator, {}), SyncHandle);
        }
    ...
    bool D3D12CommandQueue::ResolveResourceBarrierCommandList(D3D12CommandListHandle* CommandListHandle)
    {
        ...
        if (AnyResolved)
        {
            ...
            ResourceBarrierCommandListHandle->ResourceBarrier(static_cast<UINT>(ResourceBarriers.size()),
