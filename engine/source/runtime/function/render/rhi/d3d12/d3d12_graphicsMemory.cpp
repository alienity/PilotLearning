...
namespace RHI
{
    ...
    constexpr size_t PoolIndexScale =
        1; // multiply the allocation size this amount to push large values into the next bucket
...
#error Unknown forward bit-scan syntax
    ...
    {
    public:
        DeviceAllocator(_In_ ID3D12Device* device) noexcept(false) : mDevice(device)
        {
            if (!device)
            ...
            for (size_t i = 0; i < mPools.size(); ++i)
            {
                ...
                mPools[i]       = std::make_unique<RHI::LinearAllocator>(mDevice.Get(), pageSize);
            }
        }
        ...
        RHI::GraphicsResource Alloc(_In_ size_t size, _In_ size_t alignment)
        {
            ...
            return RHI::GraphicsResource(page,
...
namespace RHI
{
    ...
    {
    public:
        Impl(GraphicsMemory* owner) noexcept(false) : mOwner(owner), m_peakCommited(0), m_peakBytes(0), m_peakPages(0)
        {
#if (defined(_XBOX_ONE) && defined(_TITLE)) || defined(_GAMING_XBOX)
            if (s_graphicsMemory)
            {
    ...
    // Public constructor.
    GraphicsMemory::GraphicsMemory(_In_ D3D12LinkedDevice* Parent) :
        D3D12LinkedDeviceChild(Parent), pImpl(std::make_unique<Impl>(this))
    {
        pImpl->Initialize(Parent->GetDevice());
    }

    // Move constructor.
    GraphicsMemory::GraphicsMemory(GraphicsMemory&& moveFrom) noexcept :
        D3D12LinkedDeviceChild(moveFrom.Parent), pImpl(std::move(moveFrom.pImpl))
    {
        pImpl->mOwner = this;
    }
    ...
    void GraphicsMemory::ResetStatistics() { pImpl->ResetStatistics(); }
    ...
    GraphicsResource::GraphicsResource(_In_ LinearAllocatorPage*      page,
    ...
    SharedGraphicsResource::SharedGraphicsResource() noexcept : mSharedResource(nullptr) {}
    ...
    void SharedGraphicsResource::Reset() noexcept { mSharedResource.reset(); }
