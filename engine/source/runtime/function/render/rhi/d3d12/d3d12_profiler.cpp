...
namespace RHI
{
    ...
    {
        D3D12_QUERY_HEAP_DESC QueryHeapDesc = {D3D12_QUERY_HEAP_TYPE_TIMESTAMP, MaxProfiles * 2, Parent->GetNodeMask()};
        ...
        D3D12_HEAP_PROPERTIES HeapProperties =
            CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_READBACK, Parent->GetNodeMask(), Parent->GetNodeMask());
        D3D12_RESOURCE_DESC ResourceDesc =
            CD3DX12_RESOURCE_DESC::Buffer(static_cast<UINT64>(MaxProfiles) * FrameLatency * 2 * sizeof(UINT64));
        VERIFY_D3D12_API(Parent->GetDevice()->CreateCommittedResource(&HeapProperties,
                                                                      D3D12_HEAP_FLAG_NONE,
                                                                      &ResourceDesc,
                                                                      D3D12_RESOURCE_STATE_COPY_DEST,
                                                                      nullptr,
                                                                      IID_PPV_ARGS(&TimestampQueryReadback)));
    ...
    void D3D12Profiler::PushEventNode(D3D12CommandQueue*         CommandQueue,
    ...
    D3D12ProfileBlock::D3D12ProfileBlock(D3D12Profiler*             Profiler,
    ...
    D3D12ProfileBlock::~D3D12ProfileBlock() { Profiler->PopEventNode(CommandList); }
