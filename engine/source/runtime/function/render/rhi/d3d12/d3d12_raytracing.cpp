...
namespace RHI
{
    ...
    void D3D12RaytracingScene::ComputeMemoryRequirements(ID3D12Device5* Device,
                                                         ...
                                                         UINT64*        pResultSizeInBytes)
    {
        ...
        ScratchSizeInBytes = D3D12RHIUtils::AlignUp<UINT64>(PrebuildInfo.ScratchDataSizeInBytes,
                                                            D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT);
        ResultSizeInBytes  = D3D12RHIUtils::AlignUp<UINT64>(PrebuildInfo.ResultDataMaxSizeInBytes,
                                                           D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT);
    ...
    void D3D12RaytracingScene::Generate(ID3D12GraphicsCommandList4* CommandList,
                                        ID3D12Resource*             Scratch,
                                        ID3D12Resource*             Result,
                                        D3D12_GPU_VIRTUAL_ADDRESS   InstanceDescs)
    {
        assert(ScratchSizeInBytes > 0 && ResultSizeInBytes > 0 &&
    ...
    D3D12RaytracingMemoryPage::D3D12RaytracingMemoryPage(D3D12LinkedDevice*    Parent,
                                                         UINT64                PageSize,
        ...
        PageSize(PageSize), VirtualAddress(Resource->GetGPUVirtualAddress())
    {}

    D3D12RaytracingAllocator::D3D12RaytracingAllocator(D3D12LinkedDevice*    Parent,
                                                       D3D12_HEAP_TYPE       HeapType,
                                                       D3D12_RESOURCE_STATES InitialResourceState,
                                                       UINT64                DefaultPageSize,
                                                       UINT64                Alignment) :
        D3D12LinkedDeviceChild(Parent),
        HeapType(HeapType), InitialResourceState(InitialResourceState), DefaultPageSize(DefaultPageSize),
        Alignment(Alignment)
    {}
    ...
    {
        Pages.emplace_back(std::make_unique<D3D12RaytracingMemoryPage>(
            GetParentLinkedDevice(), PageSize, HeapType, InitialResourceState));
    }

    bool D3D12RaytracingAllocator::FindFreeSubBlock(D3D12RaytracingMemoryPage* Page,
    ...
    D3D12RaytracingManager::D3D12RaytracingManager(D3D12LinkedDevice* Parent, UINT64 PageSize) :
        D3D12LinkedDeviceChild(Parent), ScratchPool(Parent,
    ...
    UINT64 D3D12RaytracingManager::Build(ID3D12GraphicsCommandList4*                                 CommandList,
    ...
    void D3D12RaytracingManager::Release(UINT64 Index)
    {
        D3D12AccelerationStructure& AccelerationStructure = AccelerationStructures[Index];
    ...
    void D3D12RaytracingManager::Copy(ID3D12GraphicsCommandList4* CommandList)
    {
        ...
        for (const auto& Page : GpuPages)
        {
            Barriers[NumBarriers++] = CD3DX12_RESOURCE_BARRIER::Transition(
                Page->GetResource(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
        }
        ...
        {
            Barriers[NumBarriers++] = CD3DX12_RESOURCE_BARRIER::Transition(
                Page->GetResource(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        }
        CommandList->ResourceBarrier(NumBarriers, Barriers);
    }

    void D3D12RaytracingManager::Compact(ID3D12GraphicsCommandList4* CommandList, UINT64 AccelerationStructureIndex)
    {
        ...
        if (AccelerationStructure.RequestedCompaction)
        {
            ...
            if (!AccelerationStructure.IsCompacted)
            {
                ...
                D3D12ScopedMap ScopedMap(CompactedResource, 0, Range, [&](void* Data) {
    ...
    void D3D12RaytracingManager::SetSyncHandle(UINT64 AccelerationStructureIndex, D3D12SyncHandle SyncHandle)
    {
        AccelerationStructures[AccelerationStructureIndex].SyncHandle = SyncHandle;
    }

    D3D12_GPU_VIRTUAL_ADDRESS D3D12RaytracingManager::GetAddress(UINT64 AccelerationStructureIndex)
    {
                                                                        ...
                                                                        UINT BaseMissShaderIndex) const
    {
        ...
        Desc.RayGenerationShaderRecord = {BaseAddress + RayGenerationShaderTableOffset +
                                          ...
                                          RayGenerationShaderTableSizeInBytes};
        Desc.MissShaderTable = {BaseAddress + MissShaderTableOffset + BaseMissShaderIndex * MissShaderRecordStride,
                                MissShaderTableSizeInBytes,
                                MissShaderRecordStride};
        Desc.HitGroupTable   = {
            BaseAddress + HitGroupShaderTableOffset, HitGroupShaderTableSizeInBytes, HitGroupShaderRecordStride};
        return Desc;
    }
}