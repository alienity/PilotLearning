...
#pragma pack(pop)
        ...
        Microsoft::WRL::ComPtr<ID3D12PipelineState> generateMipsPSO;

        GenerateMipsResources(_In_ ID3D12Device* device)
        {
            rootSignature = CreateGenMipsRootSignature(device);
            generateMipsPSO = CreateGenMipsPipelineState(device, rootSignature.Get(), g_pGenerateMipsCS, sizeof(g_pGenerateMipsCS));
        }

    private:
        static Microsoft::WRL::ComPtr<ID3D12RootSignature> CreateGenMipsRootSignature(_In_ ID3D12Device* device)
        {
        ...
        static Microsoft::WRL::ComPtr<ID3D12PipelineState> CreateGenMipsPipelineState(_In_ ID3D12Device*                      device,
    ...
    {
    public:
        Impl(_In_ ID3D12Device* device) noexcept :
            mDevice(device), mCommandType(D3D12_COMMAND_LIST_TYPE_DIRECT), mInBeginEndBlock(false),
            mTypedUAVLoadAdditionalFormats(false), mStandardSwizzle64KBSupported(false)
        {
        ...
        void Begin(D3D12_COMMAND_LIST_TYPE commandType)
        {
            ...
            ThrowIfFailed(
                mDevice->CreateCommandAllocator(commandType, IID_PPV_ARGS(mCmdAlloc.ReleaseAndGetAddressOf())));

            SetDebugObjectName(mCmdAlloc.Get(), L"ResourceUploadBatch");

            ThrowIfFailed(mDevice->CreateCommandList(
                1, commandType, mCmdAlloc.Get(), nullptr, IID_PPV_ARGS(mList.ReleaseAndGetAddressOf())));
        ...
        void Upload(_In_ ID3D12Resource*                                      resource,
                    ...
                    uint32_t                                                  numSubresources)
        {
            ...
            ThrowIfFailed(mDevice->CreateCommittedResource(&heapProps,
        ...
        void Transition(_In_ ID3D12Resource*       resource,
        ...
        std::future<void> End(_In_ ID3D12CommandQueue* commandQueue)
        {
            if (!mInBeginEndBlock)
    ...
    private:
        ...
        void GenerateMips_UnorderedAccessPath(_In_ ID3D12Resource* resource)
        {
            ...
            if ((desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS) == 0)
            {
                ...
                stagingDesc.Format = D3D12RHIUtils::ConvertSRVtoResourceFormat(desc.Format);
            ...
            else
            {
                ...
                TransitionResource(
                    mList.Get(), staging.Get(), originalState, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            }
            ...
            mDevice->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(descriptorHeap.GetAddressOf()));

            SetDebugObjectName(descriptorHeap.Get(), L"ResourceUploadBatch");

            auto const descriptorSize =
                static_cast<int>(mDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
            ...
            mList->SetComputeRootDescriptorTable(GenerateMipsResources::SourceTexture,
                                                 descriptorHeap->GetGPUDescriptorHandleForHeapStart());

            // Get the descriptor handle -- uavH will increment over each loop
            CD3DX12_GPU_DESCRIPTOR_HANDLE uavH(descriptorHeap->GetGPUDescriptorHandleForHeapStart(),
            ...
            for (uint32_t mip = 1; mip < desc.MipLevels; ++mip)
            {
                ...
                constants.InvOutTexelSize = GenerateMipsResources::Float2 {1 / float(mipWidth), 1 / float(mipHeight)};
                mList->SetComputeRoot32BitConstants(
                    GenerateMipsResources::Constants, GenerateMipsResources::Num32BitConstants, &constants, 0);
            ...
            if (staging.Get() != resource)
            {
                ...
                barrier[0].Transition.Subresource = barrier[1].Transition.Subresource =
            ...
            {
                TransitionResource(
                    mList.Get(), staging.Get(), D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, originalState);
            }
        ...
        void GenerateMips_TexturePathBGR(_In_ ID3D12Resource* resource)
        {
            ...
            ThrowIfFailed(mDevice->CreateHeap(&heapDesc, IID_PPV_ARGS(heap.GetAddressOf())));

            SetDebugObjectName(heap.Get(), L"ResourceUploadBatch");

            Microsoft::WRL::ComPtr<ID3D12Resource> resourceCopy;
            ThrowIfFailed(mDevice->CreatePlacedResource(heap.Get(),
                                                        0,
                                                        &copyDesc,
                                                        D3D12_RESOURCE_STATE_COPY_DEST,
                                                        nullptr,
            ...
            aliasBarrier[1].Transition.Subresource      = aliasBarrier[2].Transition.Subresource =
                D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        ...
        struct UploadBatch
        {
            std::vector<Microsoft::WRL::ComPtr<ID3D12DeviceChild>> TrackedObjects;
	...
	// Public constructor.
    ResourceUploadBatch::ResourceUploadBatch(_In_ D3D12LinkedDevice* Parent) noexcept(false) :
        D3D12LinkedDeviceChild(Parent), pImpl(std::make_unique<Impl>(Parent->GetDevice()))
    {}
    ...
    void ResourceUploadBatch::Begin(D3D12_COMMAND_LIST_TYPE commandType) { pImpl->Begin(commandType); }
    ...
    _Use_decl_annotations_ void ResourceUploadBatch::Transition(ID3D12Resource*       resource,
