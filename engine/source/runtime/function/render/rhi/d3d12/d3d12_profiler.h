...
namespace RHI
{
    ...
    struct D3D12EventNode
    {
        ...
        explicit D3D12EventNode(D3D12Profiler* Profiler, INT Depth, std::string_view Name, D3D12EventNode* Parent) :
            Profiler(Profiler), Depth(Depth), Name(Name), Parent(Parent)
        {}
        ~D3D12EventNode()
        {
        ...
        {
            if (auto Iterator = Lut.find(Name); Iterator != Lut.end())
            {
    ...
    public:
        ...
        void
        PushEventNode(D3D12CommandQueue* CommandQueue, std::string_view Name, ID3D12GraphicsCommandList* CommandList);
    ...
    {
    public:
        D3D12ProfileBlock(D3D12Profiler*             Profiler,
