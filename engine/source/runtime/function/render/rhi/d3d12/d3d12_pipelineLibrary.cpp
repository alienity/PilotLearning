...
namespace RHI
{
        ...
        MappedFile(Stream), MappedView(MappedFile.CreateView())
    {
        ...
        if (Device1)
        {
            ...
            std::byte* LibraryBlob = MappedView.GetView(sizeof(SIZE_T));

            // Create a Pipeline Library from the serialized blob.
            // Note: The provided Library Blob must remain valid for the lifetime of the object returned - for
            // efficiency, the data is not copied.
            HRESULT Result = Device1->CreatePipelineLibrary(LibraryBlob, BlobLength, IID_PPV_ARGS(&PipelineLibrary1));
            switch (Result)
            {
                case DXGI_ERROR_UNSUPPORTED: // The driver doesn't support Pipeline libraries. WDDM2.1 drivers must
                                             // support it.
                    break;

                case E_INVALIDARG:                  // The provided Library is corrupted or unrecognized.
                case D3D12_ERROR_ADAPTER_NOT_FOUND: // The provided Library contains data for different hardware (Don't
                                                    // really need to clear the cache, could have a cache per adapter).
                case D3D12_ERROR_DRIVER_VERSION_MISMATCH: // The provided Library contains data from an old driver or
                                                          // runtime. We need to re-create it.
    ...
    D3D12PipelineLibrary::~D3D12PipelineLibrary()
    {
        ...
        if (!ShouldInvalidateDiskCache)
        {
            if (PipelineLibrary1)
            {
                // Important: An ID3D12PipelineLibrary object becomes undefined when the underlying memory, that was
                // used to initalize it, changes.
                ...
                if (SerializedSize > 0)
                {
                    ...
                    else
                    {
                        ...
                        MappedView.Write<SIZE_T>(0, SerializedSize);
                        HRESULT OpResult =
                            PipelineLibrary1->Serialize(MappedView.GetView(sizeof(SIZE_T)), SerializedSize);
                        if (FAILED(OpResult))
                        {
                            LOG_WARN("Failed to serialize pipeline library");
                        }
        ...
        else
        {
            ...
            Stream.reset();
            BOOL OpResult = DeleteFileW(Path.c_str());
            assert(OpResult);
        }
    }
}
