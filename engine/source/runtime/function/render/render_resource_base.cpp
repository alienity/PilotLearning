...
namespace Pilot
{
    ...
    RenderMeshData RenderResourceBase::loadMeshData(const MeshSourceDesc& source, AxisAlignedBox& bounding_box)
    {
        ...
        if (std::filesystem::path(source.m_mesh_file).extension() == ".json")
        {
        ...
        for (size_t s = 0; s < shapes.size(); s++)
        {
            ...
            for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
            {
                ...
                if (!with_normal)
                {
                    ...
                    normal[0]  = Vector3::cross(v0, v1); // v0.crossProduct(v1).normalisedCopy();
                    normal[1]  = normal[0];
                    normal[2]  = normal[0];
                }
                ...
                Vector3 tangent {1, 0, 0};
                {
                    ...
                    tangent   = Vector3::normalize(tangent); //(tangent).normalisedCopy();
                }
