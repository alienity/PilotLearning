...
namespace Pilot
{
    struct PointLight
    {
        ...
        float calculateRadius() const
        {
            ...
            float       maxIntensity        = MOYU_MAX3(intensity.x, intensity.y, intensity.z);
            float       attenuation = MOYU_MAX(INTENSITY_CUTOFF, ATTENTUATION_CUTOFF * maxIntensity) / maxIntensity;
