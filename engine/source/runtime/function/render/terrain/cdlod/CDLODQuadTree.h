#ifndef CDLOD_QUADTREE_H
#define CDLOD_QUADTREE_H
...
namespace MoYu
{
    ...
    public:
        ...
        private:
            ...
            glm::float3   m_observerPos;
            float         m_visibilityDistance;
            Frustum       m_frustum;
            float         m_LODDistanceRatio;
            float m_morphStartRatio; // [0, 1] - when to start morphing to the next (lower-detailed) LOD level; default
                                     // is 0.667 - first 0.667 part will not be morphed, and the morph will go from
                                     // 0.667 to 1.0
        ...
        public:
            LODSelection(SelectedNode*      selectionBuffer,
        ...
        public:
            LODSelectionOnStack(const glm::float3& observerPos,
                                float              visibilityDistance,
                                Plane              frustumPlanes[6],
                                float              LODDistanceRatio,
                                float              morphStartRatio = 0.66f,
                                bool               sortByDistance  = false) :
                LODSelection(m_selectionBufferOnStack,
                             maxSelectionCount,
                             observerPos,
                             visibilityDistance,
                             frustumPlanes,
        ...
        struct Node
        {
            ...
            // When IsLeaf() these can be reused for something else (currently they store float heights for ray triangle
            // test but that could be stored in a matrix without 4x redundancy like here) Also, these could/should be
            // indices into CDLODQuadTree::m_allNodesBuffer - no additional memory will then be used if compiled for
            // 64bit, and they could also be unsigned short-s if having less 65535 nodes
            Node* SubTL;
            Node* SubTR;
            Node* SubBL;
            Node* SubBR;
            ...
            void GetAABB(AABB& aabb, int rasterSizeX, int rasterSizeY, const MapDimensions& mapDims) const;
            void GetBSphere(glm::float3& sphereCenter, float& sphereRadiusSq, const CDLODQuadTree& quadTree) const;
        ...
        private:
            ...
            void            GetAreaMinMaxHeight(int                  fromX,
                                                int                  fromY,
                                                int                  toX,
                                                int                  toY,
                                                float&               minZ,
            ...
            bool IntersectRay(const glm::float3&   rayOrigin,
    ...
    void inline CDLODQuadTree::Node::GetBSphere(glm::float3& sphereCenter, float& sphereRadiusSq, const CDLODQuadTree& quadTree) const
    {
    ...
    {
        GetWorldMinMaxX(aabb.m_min.x, aabb.m_max.x, rasterSizeX, mapDims);
        GetWorldMinMaxY(aabb.m_min.y, aabb.m_max.y, rasterSizeY, mapDims);
        GetWorldMinMaxZ(aabb.m_min.z, aabb.m_max.z, mapDims);
    }
    ...
    {
        aabb.m_min.x = mapDims.MinX + this->X * mapDims.SizeX / (float)(rasterSizeX - 1);
        aabb.m_max.x = mapDims.MinX + (this->X + this->Size) * mapDims.SizeX / (float)(rasterSizeX - 1);
        aabb.m_min.y = mapDims.MinY + this->Y * mapDims.SizeY / (float)(rasterSizeY - 1);
        aabb.m_max.y = mapDims.MinY + (this->Y + this->Size) * mapDims.SizeY / (float)(rasterSizeY - 1);
        aabb.m_min.z = mapDims.MinZ + this->MinZ * mapDims.SizeZ / 65535.0f;
        aabb.m_max.z = mapDims.MinZ + this->MaxZ * mapDims.SizeZ / 65535.0f;
    }

    inline CDLODQuadTree::SelectedNode::SelectedNode(const Node* node,
                                                     int         LODLevel,
                                                     bool        tl,
                                                     bool        tr, 
                                                     bool        bl,
