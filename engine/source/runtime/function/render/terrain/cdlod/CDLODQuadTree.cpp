...
namespace MoYu
{
    ...
    bool CDLODQuadTree::Create(const CreateDesc& desc)
    {
        ...
        for (int y = 0; y < m_topNodeCountY; y++)
        {
            ...
            for (int x = 0; x < m_topNodeCountX; x++)
            {
                ...
                m_topLevelNodes[y][x]->Create(
                    x * m_topNodeSize, y * m_topNodeSize, m_topNodeSize, 0, m_desc, m_allNodesBuffer, nodeCounter);
            }
        }
    ...
    void CDLODQuadTree::Node::Create(int               x,
                                     int               y,
                                     int               size,
                                     int               level,
                                     ...
                                     int&              allNodesBufferLastIndex)
    {
        ...
        if (size == (createDesc.LeafRenderNodeSize))
        {
            ...
            int limitX = glm::min(rasterSizeX, x + size + 1);
            int limitY = glm::min(rasterSizeY, y + size + 1);
            ...
            // (reuse otherwise empty pointers used for sub nodes)
            {
                ...
                int limitX = glm::min(rasterSizeX - 1, x + size);
                int limitY = glm::min(rasterSizeY - 1, y + size);

                *pTLZ = createDesc.MapDims.MinZ +
                        createDesc.pHeightmap->GetHeightAt(x, y) * createDesc.MapDims.SizeZ / 65535.0f;
        ...
        else
        {
            ...
            if ((x + subSize) < rasterSizeX)
            {
                ...
                this->SubTR->Create(
                    x + subSize, y, subSize, level + 1, createDesc, allNodesBuffer, allNodesBufferLastIndex);
                this->MinZ = glm::min(this->MinZ, this->SubTR->MinZ);
                this->MaxZ = glm::max(this->MaxZ, this->SubTR->MaxZ);
            }
            ...
            if ((y + subSize) < rasterSizeY)
            {
                ...
                this->SubBL->Create(
                    x, y + subSize, subSize, level + 1, createDesc, allNodesBuffer, allNodesBufferLastIndex);
                this->MinZ = glm::min(this->MinZ, this->SubBL->MinZ);
                this->MaxZ = glm::max(this->MaxZ, this->SubBL->MaxZ);
            }
            ...
            if (((x + subSize) < rasterSizeX) && ((y + subSize) < rasterSizeY))
            {
                ...
                this->SubBR->Create(
                    x + subSize, y + subSize, subSize, level + 1, createDesc, allNodesBuffer, allNodesBufferLastIndex);
                this->MinZ = glm::min(this->MinZ, this->SubBR->MinZ);
                this->MaxZ = glm::max(this->MaxZ, this->SubBR->MaxZ);
            }
        }
    }
    ...
    CDLODQuadTree::Node::LODSelectResult CDLODQuadTree::Node::LODSelect(LODSelectInfo& lodSelectInfo, bool parentCompletelyInFrustum) const
    {
        ...
        const Frustum      frustum           = lodSelectInfo.SelectionObj->m_frustum;
        const glm::float3& observerPos       = lodSelectInfo.SelectionObj->m_observerPos;
        ...
        ContainmentFlag frustumIt =
            (parentCompletelyInFrustum) ? (CONTAINMENT_CONTAINS) : (IsFrustumContainAABB(frustum, boundingBox));
        if (frustumIt == CONTAINMENT_DISJOINT)
            return IT_OutOfFrustum;

        float distanceLimit = lodRanges[this->GetLevel()];

        if (!boundingBox.intersects(BSphere {observerPos, distanceLimit}))
        ...
        if (this->GetLevel() != lodSelectInfo.StopAtLevel)
        {
            ...
            if (boundingBox.intersects(BSphere {observerPos, nextDistanceLimit}))
            {
                bool weAreCompletelyInFrustum = frustumIt == CONTAINMENT_CONTAINS;

                if (SubTL != NULL)
                    SubTLSelRes = this->SubTL->LODSelect(lodSelectInfo, weAreCompletelyInFrustum);
                if (SubTR != NULL)
                    SubTRSelRes = this->SubTR->LODSelect(lodSelectInfo, weAreCompletelyInFrustum);
                if (SubBL != NULL)
                    SubBLSelRes = this->SubBL->LODSelect(lodSelectInfo, weAreCompletelyInFrustum);
                if (SubBR != NULL)
                    SubBRSelRes = this->SubBR->LODSelect(lodSelectInfo, weAreCompletelyInFrustum);
            }
        }
        ...
        // if any of child nodes are selected, then return selected - otherwise all of them are out of frustum, so we're
        // out of frustum too
    ...
    {
        const glm::float3& cameraPos          = selectionObj->m_observerPos;
        ...
        for (int i = 0; i < layerCount; i++)
        {
            ...
            selectionObj->m_morphStart[i] =
                prevPos + (selectionObj->m_morphEnd[i] - prevPos) * selectionObj->m_morphStartRatio;
        ...
        for (int i = 0; i < lodSelInfo.SelectionCount; i++)
        {
            ...
            selectionObj->m_minSelectedLODLevel =
                glm::min(selectionObj->m_minSelectedLODLevel, selectionObj->m_selectionBuffer[i].LODLevel);
            selectionObj->m_maxSelectedLODLevel =
                glm::max(selectionObj->m_maxSelectedLODLevel, selectionObj->m_selectionBuffer[i].LODLevel);
        }
        ...
        if (selectionObj->m_sortByDistance)
        {
            std::qsort(selectionObj->m_selectionBuffer,
                       selectionObj->m_selectionCount,
                       sizeof(*selectionObj->m_selectionBuffer),
                       compare_closerFirst);
        }
    }

    void CDLODQuadTree::Node::GetAreaMinMaxHeight(int                  fromX,
                                                  int                  fromY,
                                                  int                  toX,
                                                  int                  toY,
                                                  float&               minZ,
                                                  ...
                                                  const CDLODQuadTree& quadTree) const
    {
        if (((toX < this->X) || (toY < this->Y)) ||
            ((fromX > (this->X + this->Size)) || (fromY > (this->Y + this->Size))))
        {
            // Completely outside
            return;
        }
                           ...
                           ((toX >= (this->X + this->Size)) && (toY >= (this->Y + this->Size)))))
        {
            ...
            minZ = glm::min(minZ, worldMinZ);
            maxZ = glm::max(maxZ, worldMaxZ);
    ...
    void CDLODQuadTree::GetAreaMinMaxHeight(float  fromX,
                                            float  fromY,
                                            float  sizeX,
                                            float  sizeY,
                                            float& minZ,
                                            float& maxZ) const
    {
        ...
        for (int y = baseFromY; y <= baseToY; y++)
            ...
            {
                m_topLevelNodes[y][x]->GetAreaMinMaxHeight(
                    rasterFromX, rasterFromY, rasterToX, rasterToY, minZ, maxZ, *this);
            }

        // GetCanvas3D()->DrawBox( D3DXVECTOR3(fromX, fromY, minZ), D3DXVECTOR3(fromX + sizeX, fromY + sizeY, maxZ),
        // 0xFFFFFF00, 0x10FFFF00 );
    }
...
#define _IT_SUB(dest, v1, v2) \
    ...
    bool IntersectTri(const glm::float3& _orig,
                      const glm::float3& _dir,
                      const glm::float3& _vert0,
                      const glm::float3& _vert1,
                      const glm::float3& _vert2,
                      float&             u,
                      float&             v,
                      float&             dist)
    {
    ...
    bool CDLODQuadTree::Node::IntersectRay(const glm::float3&   rayOrigin,
                                           const glm::float3&   rayDirection,
                                           float                maxDistance,
                                           glm::float3&         hitPoint,
                                           const CDLODQuadTree& quadTree) const
    {
        ...
        if (IsLeaf())
        {
            ...
            glm::float3 tl(boundingBox.m_min.x, boundingBox.m_min.y, *pTLZ);
            glm::float3 tr(boundingBox.m_max.x, boundingBox.m_min.y, *pTRZ);
            glm::float3 bl(boundingBox.m_min.x, boundingBox.m_max.y, *pBLZ);
            glm::float3 br(boundingBox.m_max.x, boundingBox.m_max.y, *pBRZ);
            ...
            if (t0 && (dist0 > maxDistance))
                t0 = false;
            if (t1 && (dist1 > maxDistance))
                t1 = false;
    ...
    bool CDLODQuadTree::IntersectRay(const glm::float3& rayOrigin,
                                     const glm::float3& rayDirection,
                                     float              maxDistance,
                                     glm::float3&       hitPoint) const
    {
        float       closestHitDist = FLT_MAX;
        glm::float3 closestHit;
    ...
    CDLODQuadTree::LODSelection::LODSelection(SelectedNode*      selectionBuffer,
                                              int                maxSelectionCount,
                                              const glm::float3& observerPos,
    ...
    void CDLODQuadTree::LODSelection::GetMorphConsts(int LODLevel, float consts[4]) const
    {
        ...
        mEnd = glm::lerp(mEnd, mStart, errorFudge);
        //
        consts[0] = mStart;
