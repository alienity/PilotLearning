...
namespace MoYu
{
	namespace Geometry
	{
        BasicMesh Icosphere::ToBasicMesh(float radius, int sub, bool smooth)
        {
            BasicMesh _mesh = Icosphere(radius, sub, smooth);
        ...
        AxisAlignedBox Icosphere::ToAxisAlignedBox(const BasicMesh& basicMesh)
        {
        ...
        StaticMeshData Icosphere::ToStaticMesh(const BasicMesh& basicMesh)
        {
        ...
        StaticMeshData Icosphere::ToStaticMesh(float radius, int sub, bool smooth)
        {
            BasicMesh _basicMesh = ToBasicMesh();

            MoYu::StaticMeshData _staticMeshData = ToStaticMesh(_basicMesh);
        ...
        // ctor
        ///////////////////////////////////////////////////////////////////////////////
        Icosphere::Icosphere(float radius, int sub, bool smooth) : _radius(radius), _subdivision(sub), _smooth(smooth)
        {
            if (smooth)
                buildVerticesSmooth();
            else
            ...
            int verticeCount = _vertices.size() / 3;
        ...
        {
            float scale = computeScaleForLength(&_vertices[0], _radius);
        ...
        {
            const float H_ANGLE = MoYu::f::PI / 180 * 72;   // 72 degree = 360 / 5
            const float V_ANGLE = atanf(1.0f / 2); // elevation = 26.565 degree

            std::vector<float> vertices(12 * 3);                // 12 vertices
            int                i1, i2;                          // indices
            float              z, xy;                           // coords
            float              hAngle1 = -MoYu::f::PI / 2 - H_ANGLE / 2; // start from -126 deg at 2nd row
            float              hAngle2 = -MoYu::f::PI / 2;               // start from -90 deg at 3rd row
            ...
            for (int i = 1; i <= 5; ++i)
            {
                ...
                z  = _radius * sinf(V_ANGLE); // elevaton
                xy = _radius * cosf(V_ANGLE);
        ...
        void Icosphere::buildVerticesSmooth()
        {
            ...
            std::map<std::pair<float, float>, unsigned int>().swap(_sharedIndices);
            ...
            v[0] = tmpVertices[6];
            v[1] = tmpVertices[7];
            v[2] = tmpVertices[8]; // v14 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 2, T_STEP);
            _sharedIndices[std::make_pair(S_STEP * 2, T_STEP)] = _texCoords.size() / 2 - 1;

            v[0] = tmpVertices[9];
            v[1] = tmpVertices[10];
            v[2] = tmpVertices[11]; // v15 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 4, T_STEP);
            _sharedIndices[std::make_pair(S_STEP * 4, T_STEP)] = _texCoords.size() / 2 - 1;

            v[0]  = tmpVertices[12];
            v[1]  = tmpVertices[13];
            v[2]  = tmpVertices[14]; // v16 (shared)
            ...
            _sharedIndices[std::make_pair(S_STEP * 6, T_STEP)] = _texCoords.size() / 2 - 1;

            v[0] = tmpVertices[15];
            v[1] = tmpVertices[16];
            v[2] = tmpVertices[17]; // v17 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 8, T_STEP);
            _sharedIndices[std::make_pair(S_STEP * 8, T_STEP)] = _texCoords.size() / 2 - 1;

            v[0] = tmpVertices[21];
            v[1] = tmpVertices[22];
            v[2] = tmpVertices[23]; // v18 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 3, T_STEP * 2);
            _sharedIndices[std::make_pair(S_STEP * 3, T_STEP * 2)] = _texCoords.size() / 2 - 1;

            v[0] = tmpVertices[24];
            v[1] = tmpVertices[25];
            v[2] = tmpVertices[26]; // v19 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 5, T_STEP * 2);
            _sharedIndices[std::make_pair(S_STEP * 5, T_STEP * 2)] = _texCoords.size() / 2 - 1;

            v[0] = tmpVertices[27];
            v[1] = tmpVertices[28];
            v[2] = tmpVertices[29]; // v20 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 7, T_STEP * 2);
            _sharedIndices[std::make_pair(S_STEP * 7, T_STEP * 2)] = _texCoords.size() / 2 - 1;

            v[0] = tmpVertices[30];
            v[1] = tmpVertices[31];
            v[2] = tmpVertices[32]; // v21 (shared)
            Icosphere::computeVertexNormal(v, n);
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(S_STEP * 9, T_STEP * 2);
            _sharedIndices[std::make_pair(S_STEP * 9, T_STEP * 2)] = _texCoords.size() / 2 - 1;
