...
template <typename T>
inline MoYu::Vector4 linearToRGBM(const T& linear) {
    MoYu::Vector4 RGBM(linear[0], linear[1], linear[2], 1.0f);
...
template <typename T>
inline MoYu::Vector3 RGBMtoLinear(const T& rgbm) {
    MoYu::Vector3 linear(rgbm[0], rgbm[1], rgbm[2]);
    linear *= rgbm.a * 16.0f;
...
template <typename T>
inline MoYu::Vector3 linearTosRGB(const T& linear) {
...
template<>
inline MoYu::Vector3 sRGBToLinear(const MoYu::Vector3& sRGB) {
    using MoYu::Vector3;
    constexpr float a = 0.055f;
    constexpr float a1 = 1.055f;
...
static LinearImage toLinear(size_t w, size_t h, size_t bpr,
    ...
    auto d = result.get< MoYu::Vector3>();
    for (size_t y = 0; y < h; ++y) {
        T const* p = reinterpret_cast<T const*>(src + y * bpr);
        for (size_t x = 0; x < w; ++x, p += 3) {
             MoYu::Vector3 sRGB(proc(p[0]), proc(p[1]), proc(p[2]));
...
static LinearImage toLinear(size_t w, size_t h, size_t bpr, const uint8_t* src) {
    LinearImage result(w, h, 3);
    MoYu::Vector3* d = reinterpret_cast<MoYu::Vector3*>(result.getPixelRef(0, 0));
    for (size_t y = 0; y < h; ++y) {
        T const* p = reinterpret_cast<T const*>(src + y * bpr);
        for (size_t x = 0; x < w; ++x, p += 3) {
            MoYu::Vector3 sRGB(p[0], p[1], p[2]);
