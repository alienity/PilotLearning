...
LinearImage horizontalStack(const LinearImage* first, size_t count) {
    assert(count > 0); // "Must supply one or more images for stacking."
    ...
    for (size_t c = 0; c < count; ++c) {
        ...
        if (height == 0) {
            ...
            assert(height == img.getHeight()); // "Inconsistent heights."
        }
        if (nchannels == 0) {
            nchannels = img.getChannels();
        } else {
            assert(nchannels == img.getChannels()); // "Inconsistent channels."
        }
    }
