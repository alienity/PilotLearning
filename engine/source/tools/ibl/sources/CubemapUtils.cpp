...
void CubemapUtils::clamp(Image& src) {
    ...
    // By Brian Karis
    auto compress = [](glm::float3 color, float linear, float compressed) {
        float luma = glm::dot(color, glm::float3{ 0.2126, 0.7152, 0.0722 }); // REC 709
...
void CubemapUtils::equirectangularToCubemap(Cubemap& dst, const Image& src) {
    ...
    auto toRectilinear = [width, height](glm::float3 s) -> glm::float2 {
    ...
    process<EmptyState>(dst,
        ...
        for (size_t x=0 ; x<dim ; ++x, ++data) {
            ...
            glm::float3 c = MoYu::MYFloat3::Zero;
            for (size_t sample = 0; sample < numSamples; sample++) {
                // Generate numSamples in our destination pixels and map them to input pixels
                const glm::float2 h = hammersley(uint32_t(sample), iNumSamples);
                const glm::float3 s(dst.getDirectionFor(f, x + h.x, y + h.y));
    ...
    auto parallelJobTask = [&](size_t j0, size_t count) {
        for (size_t j = j0; j < j0 + count; j++) {
            for (size_t i = 0; i < w; i++) {
                ...
                for (size_t sample = 0; sample < numSamples; sample++) {
                    const glm::float2 u = hammersley(uint32_t(sample), 1.0f / numSamples);
            ...
            [&](EmptyState&, size_t iy, Cubemap::Face f, Cubemap::Texel* data, size_t dimension) {
                for (size_t ix = 0; ix < dimension; ++ix, ++data) {
                    ...
                    for (size_t i = 0; i < sampleCount; i++) {
                        const glm::float2 h = hammersley(uint32_t(i), 1.0f / sampleCount);
...
void CubemapUtils::mirrorCubemap(Cubemap& dst, const Cubemap& src) {
    process<EmptyState>(dst,
            [&](EmptyState&, size_t y, Cubemap::Face f, Cubemap::Texel* data, size_t dim) {
        for (size_t x=0 ; x<dim ; ++x, ++data) {
            const glm::float3 N(dst.getDirectionFor(f, x, y));
            Cubemap::writeAt(data, src.sampleAt(glm::float3{ -N.x, N.y, N.z }));
        }
    });
}
...
void CubemapUtils::generateUVGrid(Cubemap& cml, size_t gridFrequencyX, size_t gridFrequencyY) {
    ...
    CubemapUtils::process<EmptyState>(cml,
            [ & ](EmptyState&, size_t y, Cubemap::Face f, Cubemap::Texel* data, size_t dim) {
                for (size_t x = 0; x < dim; ++x, ++data) {
                    bool grid = bool(((x / gridSizeX) ^ (y / gridSizeY)) & 1);
                    Cubemap::Texel t = grid ? colors[(int)f] * uvGridHDRIntensity : glm::float3(0.f);
