...
float3x3 getWorldTangentFrame(const CommonShadingParams params) 
{ 
    return params.shading_tangentToWorld;
}

float3 getWorldPosition(const CommonShadingParams params)
{
...
float3 getWorldViewVector(const CommonShadingParams params) 
{ 
...
float3 getWorldNormalVector(const CommonShadingParams params)
{ 
    return params.shading_normal;
}

float3 getWorldGeometricNormalVector(const CommonShadingParams params)
{ 
    return params.shading_geometricNormal; 
}

float3 getWorldReflectedVector(const CommonShadingParams params)
{
    return params.shading_reflected;
}

float getNdotV(const CommonShadingParams params) 
{ 
 ...
 * @public-api
 */
float3 getNormalizedViewportCoord(const CommonShadingParams params, const FrameUniforms frameUniforms)
{
    // make sure to handle our reversed-z
...
float4 getSpotLightSpacePosition(const ShadowUniforms shadowUniforms, const CommonShadingParams params, uint index, float3 dir, float zLight)
{
 ...
 */
uint getShadowCascade(const CommonShadingParams params, const FrameUniforms frameUniforms)
{
...
float4 getCascadeLightSpacePosition(const ShadowUniforms shadowUniforms, const CommonShadingParams params, 
