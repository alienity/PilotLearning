...
void getAnisotropyPixelParams(const MaterialInputs material, const CommonShadingParams params, inout PixelParams pixel)
{
#if defined(MATERIAL_HAS_ANISOTROPY)
    float3 direction   = material.anisotropyDirection;
    pixel.anisotropy   = material.anisotropy;
    pixel.anisotropicT = normalize(params.shading_tangentToWorld * direction);
    pixel.anisotropicB = normalize(cross(getWorldGeometricNormalVector(params), pixel.anisotropicT));
#endif
}
