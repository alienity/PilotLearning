...
SamplerState LinearBorder : register(s10);
...
{
    uint m_HigherResBufIndex;
    uint m_LowerResBufIndex;
    uint m_ResultIndex;
}
...
void BlurVertically(RWTexture2D<float3> Result, uint2 pixelCoord, uint topMostIndex)
{
...
{
    Texture2D<float3>   HigherResBuf = ResourceDescriptorHeap[m_HigherResBufIndex];
    Texture2D<float3>   LowerResBuf  = ResourceDescriptorHeap[m_LowerResBufIndex];
